...
const BLENDS = {
  CM:{name:"CholMel",label:"Driven Perfectionist",desc:"You combine bold leadership with a deep need for excellence. You get things done AND get them right — but you can be hard on yourself and others."},
  ...
  CP:{name:"CholPhleg",label:"Determined Diplomat",desc:"You push hard for results but genuinely care about people. You balance drive with patience better than most leaders."},
...
const DAILY_QUOTES = {
  Choleric:   [{q:"Today, pause before you respond. Your instinct is powerful — just aim it right.",a:"Daily Reminder"},{q:"You don't have to win every argument to be respected. Pick the ones that matter.",a:"Daily Reminder"},{q:"Results matter, but so does how you make people feel along the way.",a:"Daily Reminder"},{q:"Rest is not weakness. Even the best engines need to cool down.",a:"Daily Reminder"},{q:"Asking for help is not losing control — it's smart leadership.",a:"Daily Reminder"},{q:"One act of patience today can save you from three apologies tomorrow.",a:"Daily Reminder"},{q:"The people around you move slower than you — and that's okay. Bring them with you.",a:"Daily Reminder"}],
  Melancholic:[{q:"Done and delivered beats perfect and delayed. Ship the work.",a:"Daily Reminder"},{q:"You have replayed that conversation enough. It's okay to let it go now.",a:"Daily Reminder"},{q:"Your sensitivity is not a flaw. It's the reason people trust you with their secrets.",a:"Daily Reminder"},{q:"The standards you hold others to — are you holding yourself to the same kindness?",a:"Daily Reminder"},{q:"One small, messy step forward is worth more than ten perfectly planned ones you never took.",a:"Daily Reminder"},{q:"You don't have to earn rest. You're allowed to just be today.",a:"Daily Reminder"},{q:"Not everything needs to be understood to be appreciated. Enjoy something today.",a:"Daily Reminder"}],
  Sanguine:   [{q:"Following through on one thing today will mean more than starting five new ones.",a:"Daily Reminder"},{q:"The people who love you need consistency, not just good energy. Show up steady.",a:"Daily Reminder"},{q:"Your joy is contagious. Protect it — don't give it to people who don't value it.",a:"Daily Reminder"},{q:"You don't have to perform today. The real you — quiet and unfiltered — is enough.",a:"Daily Reminder"},{q:"One quiet, focused hour today will surprise you with what you can produce.",a:"Daily Reminder"},{q:"You're allowed to say no without a long explanation. No is a complete sentence.",a:"Daily Reminder"},{q:"Before you commit to something new, finish what's already in front of you.",a:"Daily Reminder"}],
  Phlegmatic: [{q:"Speak up today — even once. The room needs to hear what you've been thinking.",a:"Daily Reminder"},{q:"Avoiding conflict doesn't make it disappear. One honest conversation clears the air.",a:"Daily Reminder"},{q:"Your loyalty is rare. Make sure the people receiving it are actually worth it.",a:"Daily Reminder"},{q:"Being comfortable is great. Being a little uncomfortable is where you grow.",a:"Daily Reminder"},{q:"You hold everything together for others. Who is holding things together for you?",a:"Daily Reminder"},{q:"Say what you need today. The people who love you can't guess — they need to hear it.",a:"Daily Reminder"},{q:"You've been putting off one thing for a while now. Today is the day to start it.",a:"Daily Reminder"}],
};
...
const ROLE_ADVICE = {
  ...
  Melancholic:{spouse:"You love with extraordinary depth. Express your affection verbally — your partner may not see what's happening in your rich inner world.",student:"You are a natural student. Watch for perfectionism paralysis. Done and good enough sometimes beats perfect and late.",employer:"As a leader, you set high standards. Be mindful that your silence can feel like disapproval. Verbalize appreciation explicitly.",employee:"You thrive with clear expectations and quality work. Ask for detailed feedback — it fuels you. Set personal deadlines to beat procrastination.",teacher:"You are a deeply gifted teacher. Your students will remember your passion for years. Make time for the struggling ones — your empathy is your gift."},
  Sanguine:   {spouse:"You are the light in your relationship. Work on consistency — your partner needs to know they can count on you even when it's not exciting.",student:"School can bore you. Find the story in every subject. Build a study routine so your energy doesn't scatter before exams.",employer:"You inspire your team like few others can. Build systems that work even when your excitement drops — structure sets your creativity free.",employee:"You energize any workplace. Ask for variety in your tasks and socialize strategically. Your enthusiasm is your biggest professional asset.",teacher:"You are unforgettable in the classroom. Your students are engaged and excited. Build in quiet time — not everyone learns at your pace."},
  Phlegmatic: {spouse:"You are the most faithful partner. Speak your needs — your quiet nature can cause resentment to build silently. Your voice matters.",student:"You are steady and dependable in school. Push yourself slightly outside comfort zones — you're more capable than you believe.",employer:"Your team trusts you completely. Learn to make hard decisions faster — your team needs your calm and your direction.",employee:"You are the backbone every team needs. Advocate for yourself more. Your contributions often go unnoticed because you don't trumpet them.",teacher:"Students feel completely safe with you. Introduce gentle challenges — your calm acceptance can sometimes shield them from necessary growth."},
};
...
function Auth({ onAuth }) {
  ...
  const handleSubmit = async () => {
    ...
    if (mode === "signup") {
      ...
      if (!pwValid)      return setError("Password doesn't meet all the requirements below.");
    }
    setLoading(true);
    ...
    if (mode === "signup") {
      if (mockUsers[email]) { setError("This email is already registered. Try signing in."); setLoading(false); return; }
      mockUsers[email] = { name, password, role };
      onAuth({ name, email, role });
    } else {
      const u = mockUsers[email];
      if (!u)                      { setError("No account found with this email."); setLoading(false); return; }
      if (u.password !== password) { setError("Incorrect password. Please try again.");  setLoading(false); return; }
      onAuth({ name: u.name, email, role: u.role });
    }
    setLoading(false);
  };
        ...
        <div className="card" style={{padding:28,display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div>
            ...
            <div style={{position:"relative"}}>
              ...
              <button onClick={()=>setShowPw(v=>!v)} type="button" style={{position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:showPw?"var(--gold)":"var(--muted)",display:"flex",alignItems:"center",padding:0,transition:"color .2s"}}>
                {showPw ? <Icon name="eye" size={18}/> : <Icon name="eyeOff" size={18}/>}
              </button>
            </div>
          </div>

          {/* Password rules — shown only on signup when user starts typing */}
...
function Quiz({ onComplete }) {
  ...
  const handleAnswer = type => {
    ...
    if (current+1 >= shuffled.length) {
      ...
      const second=sorted[1][1]>=25?sorted[1][0]:null;
      onComplete(winner,pcts,second);
      ...
      <div style={{marginBottom:32}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            {current>0 && (
              <button onClick={handleBack} style={{background:"var(--surface)",border:"1px solid var(--border)",borderRadius:10,width:36,height:36,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:"var(--muted)",transition:"all .2s",flexShrink:0}}
      ...
      <div key={current} className="fade-up" style={{flex:1,display:"flex",flexDirection:"column"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {q.options.map(([text,type],i)=>{
            const sel=prevAnswer===type;
            return (
              <div key={i} onClick={()=>handleAnswer(type)}
                style={{padding:"18px 20px",border:`1px solid ${sel?"var(--gold)":"var(--border)"}`,borderRadius:16,cursor:"pointer",background:sel?"rgba(212,168,67,0.08)":"var(--surface)",display:"flex",alignItems:"center",justifyContent:"space-between",transition:"all .2s"}}
                onMouseEnter={e=>{e.currentTarget.style.borderColor="var(--gold)";e.currentTarget.style.background="rgba(212,168,67,0.06)"}}
                onMouseLeave={e=>{e.currentTarget.style.borderColor=sel?"var(--gold)":"var(--border)";e.currentTarget.style.background=sel?"rgba(212,168,67,0.08)":"var(--surface)"}}>
                <span style={{fontSize:15,lineHeight:1.5}}>{text}</span>
                {sel ? <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--gold)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
...
function Result({ temperament, percentages, secondaryTemp, onContinue }) {
  ...
  useEffect(()=>{
    const ts=[setTimeout(()=>setStage(1),600),setTimeout(()=>setStage(2),1800),setTimeout(()=>setStage(3),2800)];
  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 24px",position:"relative",overflow:"hidden"}}>
      ...
      {stage>=1 && <div className="pop-in" style={{textAlign:"center"}}><div style={{fontSize:80,marginBottom:8,animation:"float 3s ease-in-out infinite"}}>{t.element}</div></div>}
      {stage>=2 && (
        <div className="fade-up" style={{textAlign:"center",marginTop:8}}>
          ...
          <p style={{color:t.color,fontSize:16,marginTop:8,opacity:.8}}>{t.tagline}</p>
        </div>
      )}
      {stage>=3 && (
        <div className="fade-up" style={{maxWidth:360,width:"100%",textAlign:"center",marginTop:24,padding:"0 16px"}}>
          ...
          <div className="card" style={{padding:20,marginBottom:16,borderColor:t.color+"33",textAlign:"left"}}>
            <p style={{lineHeight:1.7,color:"var(--muted)",fontSize:14}}>{blend?blend.desc:t.description}</p>
          </div>
              ...
              return (
                <div key={name} style={{marginBottom:12}}>
                  ...
                  <div style={{height:8,background:"var(--surface2)",borderRadius:4,overflow:"hidden"}}>
                    <div style={{height:"100%",width:`${pct}%`,background:tc.color,borderRadius:4,transition:"width 1s ease",opacity:.85}}/>
                  </div>
                </div>
              );
            })}
...
function Dashboard({ user, temperament, percentages, secondaryTemp, isPremium, onUpgrade, onRetake }) {
  ...
  const tabs=[{id:"overview",label:"Overview"},{id:"traits",label:"Traits"},{id:"advice",label:"Advice"}];
  ...
  return (
    <div style={{maxWidth:480,margin:"0 auto",padding:"24px 16px 100px"}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:16}}>
        {tabs.map(tb=><button key={tb.id} onClick={()=>setTab(tb.id)} style={{flex:1,padding:"10px",border:`1px solid ${tab===tb.id?t.color:"var(--border)"}`,borderRadius:12,background:tab===tb.id?t.bg:"transparent",color:tab===tb.id?t.color:"var(--muted)",cursor:"pointer",fontSize:13,fontFamily:"var(--font-body)",transition:"all .2s"}}>{tb.label}</button>)}
      </div>
        ...
        <div className="fade-in">
          ...
          <div className="card" style={{padding:20}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
              {t.career.map(c=><span key={c} style={{padding:"6px 14px",background:"var(--surface2)",borderRadius:20,fontSize:13}}>{c}</span>)}
            </div>
          </div>
        </div>
      )}
      ...
      {tab==="traits" && (
        <div className="fade-in" style={{display:"flex",flexDirection:"column",gap:12}}>
          <div className="card" style={{padding:20}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              {t.strengths.map(s=><div key={s} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 12px",background:"rgba(74,222,128,0.06)",borderRadius:10,border:"1px solid rgba(74,222,128,0.15)"}}><Icon name="check" size={14}/><span style={{fontSize:13}}>{s}</span></div>)}
            </div>
          </div>
          <div className="card" style={{padding:20}}>
            <h3 style={{fontFamily:"var(--font-display)",fontSize:20,marginBottom:16,color:"#f87171"}}>⚠ Weaknesses</h3>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              {t.weaknesses.map(w=><div key={w} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 12px",background:"rgba(248,113,113,0.06)",borderRadius:10,border:"1px solid rgba(248,113,113,0.15)"}}><span style={{fontSize:13,color:"var(--muted)"}}>{w}</span></div>)}
            </div>
          </div>
        </div>
      )}
  ...
  const send = async () => {
    ...
    try {
      const res = await fetch("/api/chat",{
        ...
        body:JSON.stringify({
          system:`You are the Temperiq AI guide. Keep it simple, warm, and real — use everyday language. The user's name is ${user.name}. Their temperament is ${temperament}. Their role is ${user.role}. Talk like a wise friend who knows them well. Give practical, relatable advice they can use today. Keep replies short — 2-3 sentences max unless they ask for more. Use their traits (strengths: ${t.strengths.slice(0,3).join(", ")}; watch-outs: ${t.weaknesses.slice(0,3).join(", ")}) to make it personal.`,
          messages:newMsgs,
        }),
      });
      ...
      if (data.content && Array.isArray(data.content)) {
        reply = data.content.map(c=>c.type==="text"?c.text:"").filter(Boolean).join("") || reply;
      }
    ...
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 64px)",maxWidth:480,margin:"0 auto"}}>
      ...
      <div style={{flex:1,overflow:"auto",padding:"16px"}}>
        {messages.map((m,i)=>(
          <div key={i} style={{marginBottom:12,display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
            <div className={m.role==="user"?"bubble-user":"bubble-ai"} style={{maxWidth:"82%",padding:"12px 16px",fontSize:14,lineHeight:1.6}}>{m.content}</div>
          </div>
        ))}
        ...
        {messages.length===1 && (
          <div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:12}}>
            {prompts.map(p=><button key={p} onClick={()=>setInput(p)} style={{padding:"8px 14px",background:"var(--surface2)",border:"1px solid var(--border)",borderRadius:20,color:"var(--muted)",fontSize:12,cursor:"pointer",fontFamily:"var(--font-body)"}}>{p}</button>)}
          </div>
        )}
        <div ref={bottomRef}/>
      </div>
      {limitHit && (
        <div style={{margin:"0 16px 12px",padding:18,background:"rgba(212,168,67,0.06)",border:"1px solid rgba(212,168,67,0.2)",borderRadius:16,textAlign:"center"}}>
          <p style={{fontSize:14,color:"var(--gold)",marginBottom:4,fontWeight:600}}>You've used your {FREE_MSG_LIMIT} free messages</p>
          <p style={{fontSize:13,color:"var(--muted)",marginBottom:12}}>Upgrade to keep chatting anytime.</p>
      ...
      <div style={{padding:"12px 16px",borderTop:"1px solid var(--border)",display:"flex",gap:10}}>
        ...
        <button onClick={send} disabled={loading||!input.trim()||limitHit} style={{width:48,height:48,borderRadius:14,background:"var(--gold)",border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",opacity:input.trim()&&!loading&&!limitHit?1:.5,flexShrink:0,transition:"opacity .2s"}}>
...
function PremiumModal({ onClose, onPurchase }) {
  const plans=[
    {id:"basic",   name:"Personal",     price:"$1.99",period:"/mo",color:"#c8a02a",features:["Full trait analysis","Daily quotes","Role-based advice","Chatbot — unlimited"]},
          ...
          {plans.map(plan=>(
            <div key={plan.id} style={{minWidth:255,flex:"0 0 255px",padding:20,border:`2px solid ${plan.popular?plan.color:"var(--border)"}`,borderRadius:20,background:plan.popular?`${plan.color}14`:"var(--surface2)",position:"relative",scrollSnapAlign:"start"}}>
              ...
              <div style={{marginBottom:14}}>
                ...
                <span style={{fontFamily:"var(--font-display)",fontSize:28,fontWeight:700}}>{plan.price}</span>
              ...
              <button onClick={()=>onPurchase(plan.id)} style={{width:"100%",padding:"12px",borderRadius:12,border:`1px solid ${plan.color}`,background:plan.popular?plan.color:"transparent",color:plan.popular?"white":plan.color,cursor:"pointer",fontFamily:"var(--font-body)",fontWeight:600,fontSize:14,transition:"all .2s"}}>
                Get {plan.name}
              </button>
            </div>
          ))}
      ...
      <div className="card fade-up" style={{padding:20,marginBottom:12}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
            ...
            <p style={{fontSize:13,color:"var(--muted)",marginTop:4}}>{isPremium?"Full access unlocked ✓":`${FREE_MSG_LIMIT} free guide messages · Basic features`}</p>
          </div>
      ...
      <div className="card fade-up" style={{marginBottom:12,overflow:"hidden"}}>
        {menu.map((item,idx)=>(
          <button key={item.label} onClick={item.fn}
            style={{display:"flex",alignItems:"center",gap:14,padding:"16px 20px",background:"none",border:"none",borderBottom:idx<menu.length-1?"1px solid var(--border)":"none",cursor:"pointer",color:"var(--text)",fontFamily:"var(--font-body)",fontSize:15,width:"100%",textAlign:"left",transition:"background .15s"}}
...
export default function App() {
  ...
  const handleQuizDone    = (t,p,s) => { setTemperament(t); setPercentages(p); setSecondary(s); setScreen("result"); };
  const handleResultDone  = ()  => setScreen("app");
  const handleLogout      = ()  => { setUser(null); setTemperament(null); setPercentages(null); setSecondary(null); setIsPremium(false); setScreen("onboarding"); };
